...
st.markdown("**Real-time prices + supplies for traders & investors** | CoinGecko Data")

# Sidebar
vs_currency = st.sidebar.selectbox(
    "Select Currency (updates instantly)", 
...
def get_crypto_data(currency):
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            st.error("Rate limit – wait 60s and click Refresh")
            return pd.DataFrame()
...
if not df.empty:
    ...
    df_display = df_display.rename(columns={
        ...
        "max_supply": "Max Supply", "ath": "All-Time High",
        "ath_change_percentage": "% from ATH"
    })

    st.subheader(f"📋 Top Cryptos in {vs_currency.upper()}")
    search = st.text_input("🔍 Search coin (name or symbol)", key="search")
    ...
    st.dataframe(
        ...
        width="stretch", height=700
    )

    # Detailed view
    st.subheader("📊 Coin Details")
    selected = st.selectbox("Pick coin", df["name"].tolist())
    row = df[df["name"] == selected].iloc[0]
    col1, col2, col3 = st.columns(3)
    with col1:
        st.image(row["image"], width=120)
        st.metric("Price", f"${row['current_price']:,.4f}", f"{row['price_change_percentage_24h']:+.2f}%")
    with col2:
        st.write(f"**Circulating**: {row.get('circulating_supply', 'N/A'):,}")
        st.write(f"**Total**: {row.get('total_supply', 'N/A'):,}")
        st.write(f"**Max**: {row.get('max_supply', 'N/A'):,}")
    with col3:
        st.write(f"**Market Cap**: ${row.get('market_cap', 'N/A'):,}")
        st.write(f"**24h Vol**: ${row.get('total_volume', 'N/A'):,}")
        st.write(f"**ATH**: ${row.get('ath', 'N/A'):,}")

if st.button("🔄 Refresh Data"):
    st.rerun()

st.success("✅ Full list always visible | Search & currency work live")
